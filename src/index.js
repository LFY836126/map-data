...
// 获取行政区域ID
logLog('1、获取行政区域列表...');
httpsGet("https://geo.datav.aliyun.com/areas_v3/bound/all.json", function (err, data) {
	...
	var geoJsonUrls = list.map(function (item) {
		if (item.level !== "district")
			return {
				item: item,
				url: "https://geo.datav.aliyun.com/areas_v2/bound/" + item.adcode + (isFull ? "_full" : "") + ".json"
			};
		return {
			item: item
		};
...
const provinces = [
	...
	350000, 360000, 370000, 410000,
	420000, 430000, 440000, 450000,
	460000, 500000, 510000, 520000,
	530000, 540000, 610000, 620000,
	630000, 640000, 650000,
	810000, 820000
]

const SpecialCity = [441900, 442000, 460400, 620200, 710000];